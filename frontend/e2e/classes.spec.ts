...
test('persists class selection and keeps class links scoped', async ({ page }, testInfo) => {
  ...
  const currentStudentName = attendanceDialog.getByRole('heading', { name: 'Alex Rivera' })
  ...
  await expect(attendanceDialog.getByRole('heading', { name: 'Jamie Santos' })).toBeVisible()
  await attendanceDialog.getByRole('button', { name: 'Previous' }).click()
  await expect(attendanceDialog.getByRole('heading', { name: 'Alex Rivera' })).toBeVisible()
  ...
  await attendanceDialog.getByRole('button', { name: 'Present', exact: true }).click()
  const secondStudentName = attendanceDialog.getByRole('heading', { name: 'Jamie Santos' })
  ...
  await attendanceDialog.getByLabel('Jump to student').selectOption({ label: 'Jamie Santos' })
  await expect(attendanceDialog.getByRole('heading', { name: 'Jamie Santos' })).toBeVisible()
  ...
  const letterRail = attendanceDialog.getByRole('navigation', { name: 'Jump to students by last name' })
  await expect(letterRail.getByRole('button')).toHaveCount(27)
  ...
  await attendanceDialog.getByRole('button', { name: 'Confirm Excused' }).click()
  await expect(attendanceDialog).toContainText('Enter an excuse reason.')
  await attendanceDialog.getByLabel('Excuse reason for Alex Rivera').fill('School activity')
  await attendanceDialog.getByRole('button', { name: 'Confirm Excused' }).click()
...
test('creates, edits, clears, and deletes a class score sheet', async ({ page }) => {
  ...
  const jamieSearchResult = dialog.getByRole('option').filter({ hasText: 'Jamie Santos' })
  await expect(jamieSearchResult).toContainText('Pending')
  await studentSearch.press('ArrowDown')
  await studentSearch.press('Enter')
  await expect(dialog.getByRole('heading', { name: 'Jamie Santos' })).toBeVisible()
  await expect(dialog.getByLabel('Score for Jamie Santos')).toBeFocused()
  ...
  const alexSearchResult = dialog.getByRole('option').filter({ hasText: 'Alex Rivera' })
  await expect(alexSearchResult).toContainText('Pending')
  await alexSearchResult.click()
  await expect(dialog.getByRole('heading', { name: 'Alex Rivera' })).toBeVisible()
  await expect(dialog.getByLabel('Score for Alex Rivera')).toBeFocused()
  ...
  await expect(dialog.getByRole('heading', { name: 'Jamie Santos' })).toBeVisible()
  await expect(dialog.getByLabel('Score for Jamie Santos')).toBeEnabled()
  await dialog.getByLabel('Score for Jamie Santos').fill('7')
  await dialog.getByRole('button', { name: 'Record score' }).click()
  ...
  await expect(dialog.getByRole('heading', { name: 'Alex Rivera' })).toBeVisible()
  await expect(dialog.getByLabel('Score for Alex Rivera')).toHaveValue('')
  ...
  await dialog.getByLabel('Score for Alex Rivera').fill('8.5')
  await dialog.getByLabel('Score for Alex Rivera').press('Enter')
  await expect(dialog.getByRole('heading', { name: 'Jamie Santos' })).toBeVisible()
  await expect(dialog.getByLabel('Score for Jamie Santos')).toBeFocused()
  ...
  await expect(reopenedDialog.getByRole('heading', { name: 'Jamie Santos' })).toBeVisible()
  ...
  await expect(reopenedDialog.getByRole('heading', { name: 'Alex Rivera' })).toBeVisible()
  await expect(reopenedDialog.getByLabel('Score for Alex Rivera')).toHaveValue('8.50')
  await expect(reopenedDialog).toContainText('12 points')
  await reopenedDialog.getByLabel('Score for Alex Rivera').fill('9')
  ...
  const clearConfirmation = reopenedDialog.getByRole('alertdialog', { name: /Clear Alex Rivera's saved result/ })
  ...
  await expect(reopenedDialog.getByLabel('Score for Alex Rivera')).toHaveValue('')
  await reopenedDialog.getByRole('button', { name: 'Undo last' }).click()
  await expect(reopenedDialog.getByLabel('Score for Alex Rivera')).toHaveValue('9.00')
  await reopenedDialog.getByRole('button', { name: 'Next student' }).click()
  await expect(reopenedDialog.getByLabel('Score for Jamie Santos')).toHaveValue('0.00')
  await reopenedDialog.getByRole('tab', { name: 'Score sheets' }).click()
  ...
  await expect(mobileDialog.getByLabel('Score for Alex Rivera')).toBeVisible()
