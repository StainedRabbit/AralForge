...
test('creates and reopens an attendance-style class score sheet', async ({ page }) => {
  ...
  await expect(dialog).toContainText('1 saved')
  ...
  await reopenedDialog.getByRole('button', { name: 'Confirm excused' }).click()
  await expect(reopenedDialog.getByText('Score sheet complete')).toBeVisible()
  ...
  await reopenedDialog.getByRole('button', { name: 'Next' }).click()
  ...
  await mobileDialog.getByRole('button', { name: /Quiz 1/ }).click()
