...
test('shows the AralForge identity and migrates legacy browser storage', async ({ page, request }) => {
  ...
  const loginCardLogo = page.locator('.login-card .brand')
  await expect(loginCardLogo.locator('img.brand__image--light[src="/brand/aralforge-logo-horizontal.png"]')).toBeVisible()
  await expect(loginCardLogo.locator('img.brand__image--dark[src="/brand/aralforge-logo-horizontal-dark.png"]')).toBeHidden()
  await page.emulateMedia({ colorScheme: 'dark' })
  await expect(page.locator('html')).toHaveAttribute('data-effective-theme', 'dark')
  await expect(loginCardLogo.locator('img.brand__image--light[src="/brand/aralforge-logo-horizontal.png"]')).toBeHidden()
  await expect(loginCardLogo.locator('img.brand__image--dark[src="/brand/aralforge-logo-horizontal-dark.png"]')).toBeVisible()
