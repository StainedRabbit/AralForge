...
test('sidebar preview keeps icon geometry stable across desktop widths', async ({ page }) => {
  ...
  for (const [viewportWidth, expandedWidth] of [[1440, 268], [1024, 246]]) {
    ...
    await expect(sidebar.locator('.user-chip__logout')).toBeHidden()
    await expect(sidebar.locator('.sidebar__appearance')).toBeHidden()
    ...
    await expect.poll(() => sidebar.locator('.nav-link').first().evaluate((link) => link.getBoundingClientRect().height)).toBe(34)
