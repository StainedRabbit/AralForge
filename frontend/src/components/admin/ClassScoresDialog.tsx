...
function ScoreTable({ drafts, maximum, onChange, readOnly = false, saving }: {
  ...
  return <div className="table-wrap class-score-table-wrap"><table className="admin-table class-score-table"><thead><tr><th>Student</th><th>Score</th><th>Status</th><th>Remarks / excuse reason</th></tr></thead><tbody>{drafts.map((draft) => {
    const error = readOnly ? '' : scoreDraftError(draft, maximum)
    return <tr key={draft.student}><td><strong>{draft.studentName}</strong><span>{draft.studentNumber}</span></td><td><input aria-describedby={error && draft.status === 'GRADED' ? `score-error-${draft.student}` : undefined} aria-invalid={Boolean(error && draft.status === 'GRADED')} aria-label={`Score for ${draft.studentName}`} disabled={readOnly || saving || draft.status === 'EXCUSED'} max={maximum} min="0" onChange={(event) => onChange(draft.student, { rawScore: event.target.value })} placeholder="0" step="0.01" type="number" value={draft.rawScore} />{error && draft.status === 'GRADED' ? <small className="class-score-field-error" id={`score-error-${draft.student}`}>{error}</small> : null}</td><td><select aria-label={`Status for ${draft.studentName}`} disabled={readOnly || saving} onChange={(event) => onChange(draft.student, { status: event.target.value as ScoreStatus })} value={draft.status}><option value="GRADED">Graded</option><option value="EXCUSED">Excused</option></select></td><td><input aria-describedby={error && draft.status === 'EXCUSED' ? `remarks-error-${draft.student}` : undefined} aria-invalid={Boolean(error && draft.status === 'EXCUSED')} aria-label={`Remarks for ${draft.studentName}`} disabled={readOnly || saving} onChange={(event) => onChange(draft.student, { remarks: event.target.value })} placeholder={draft.status === 'EXCUSED' ? 'Excuse reason required' : 'Optional remarks'} required={draft.status === 'EXCUSED'} value={draft.remarks} />{error && draft.status === 'EXCUSED' ? <small className="class-score-field-error" id={`remarks-error-${draft.student}`}>{error}</small> : null}</td></tr>
  })}</tbody></table></div>
}
